...
from .helpers import divide_safe, get_ss, get_pollutants_any_year,\
                     ...
                     get_percentages_from_yearprod2,\
                     get_percentages_from_yearprod3, handle_slider,\
                     handle_slider_1, handle_slider_2, calc_workload,\
                     get_pollutants, get_co2_for_plant_by_year,\
                     get_energy_for_plant, get_co2_for_plant_by_years
from .constants import API_KEY, SORT_CRITERIA_BLOCKS, SLIDER_1, SLIDER_2p,\
                       SLIDER_2b, DEFAULT_OPS, SELECT_CHP, SELECT_CHP_LIST,\
                       FEDERAL_STATES, SOURCES_LIST, OPSTATES, HOURS_IN_YEAR, YEAR, LATEST_YEAR,\
...
def initialize_form(request, default=SORT_CRITERIA_BLOCKS, plants=False):
    slider1 = SLIDER_1
    ...
    if plants:
        ...
        sl2 = SLIDER_2b
    slider2 = sl2
    sort_criteria = default[1]
    sort_method = "-"
    search_federalstate = []
    search_power = ['Braunkohle', "Steinkohle", "Kernenergie", OIL, "Erdgas"]
    search_opstate = DEFAULT_OPS
    search_chp = []

    if request.method == 'POST':
        form = BlocksForm(request.POST)
        sort_criteria = request.POST.get('sort_by', default[1])
        search_federalstate = request.POST.getlist('select_federalstate', [])
        search_power = request.POST.getlist('select_powersource', [])
        search_opstate = request.POST.getlist('select_opstate', DEFAULT_OPS)
...
def forge_sources_plant(annotated_plants):
    ...
    for source in SOURCES_LIST:
        ...
        energy = filtered.aggregate(Sum('energy_2024'))['energy_2024__sum'] or 0
        co2 = filtered.aggregate(Sum('co2_2024'))['co2_2024__sum'] or 0
...
def annotate_plants(plants):
    ...
    eff24=Case(
        When(energy_2024=0, then=0),
        When(co2_2024=0, then=0),
        default=(F('co2_2024') / F('energy_2024')), output_field=FloatField()),
    eff=Case(
        When(energy_2024=0, then=0),
        When(co2_2024=0, then=0),
        default=(F('co2_2024') / F('energy_2024')), output_field=FloatField()),
    ...
    workload24=Case(
        When(energy_2024=0, then=0),
        default=(F('energy_2024') / (F(power_type) * HOURS_IN_YEAR) * 100),
        output_field=FloatField()),
    workload=Case(
        When(energy_2024=0, then=0),
        default=(F('energy_2024') / (F(power_type) * HOURS_IN_YEAR) * 100),
    ...
    energy=Case(
        When(energy_2024=0, then=0),
        default=(F('energy_2024') / float(10**6)), output_field=FloatField()),
    co2=Case(
        When(co2_2024=0, then=0),
        default=(F('co2_2024') / float(10**9)), output_field=FloatField()),
    )
...
def get_elist(plantid, plant):
    ...
    workload2 = [divide_safe(e,
    (plant.totalpower * HOURS_IN_YEAR)) * 100 for e in e2s]
    workload = [divide_safe(e,
    (p * HOURS_IN_YEAR)) * 100 for (e, p) in zip(e2s, powers)]
